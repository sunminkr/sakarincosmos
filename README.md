...
공식 Instagram·YouTube·SoundCloud 계정을 연결했습니다. 공개 목록에서 선별한 SNS 게시물을 `data/media.json`에서 관리합니다. 아카이브는 전체, 음악 페이지는 음악·영상을 최신순으로 표시하며 홈도 같은 목록을 사용합니다. 필요 없는 항목을 삭제하거나 `enabled`를 `false`로 바꾸면 모든 페이지에서 제외됩니다. Instagram은 데스크톱 400px, 모바일은 사진 3:4 비율에 프로필 높이를 더한 미리보기로 표시하고 전체 내용은 원본 링크로 연결합니다. 모바일 SoundCloud는 커버를 배경으로 표시하는 플레이어를 사용해 재생 파형의 가로 공간을 확보합니다. 직접 음원 파일은 연결하지 않습니다. [콘텐츠 관리 안내](docs/MEDIA.md)를 참고하세요.
