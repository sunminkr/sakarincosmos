...
현재 상품은 `sakarin cosmos logo t-shirt` 한 종류이며 가격은 25,000원입니다. Objects에는 정면 대표 사진과 펼쳐 볼 수 있는 로고·착용 상세 사진을 표시합니다. 원본은 `assets/images/products/sakarin-cosmos-logo-t-shirt/`에 보관하며 사진을 누르면 크게 볼 수 있습니다. 사이즈·색상 옵션·소재는 확인된 정보가 없어 표시하지 않습니다. 사이즈가 확정되면 `data/catalog.json`의 `sizes`에 추가할 수 있습니다. 사이즈별 담기·변경 기능은 유지합니다. 상품+사이즈별 수량은 1–9개이며, 선택이 같은 항목은 합쳐집니다. 삭제된 예시 상품은 이전 장바구니에서도 제외됩니다. 신청 메일에는 상품별 옵션·수량이 포함되고, 발송에 성공한 항목만 장바구니에서 차감합니다. 실패하면 그대로 보관합니다.
