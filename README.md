...
음악 재생은 앞으로 SoundCloud 임베드로 구성합니다. 현재 데모 플레이어는 유지하며, 직접 음원 파일 연결은 작업 대상에서 제외합니다.

## 다국어 준비 상태

현재 본문은 한국어 기준이며 공통 문구에 ko/en/ja 사전을 적용했습니다. 영어·일본어 본문 페이지는 아직 공개하지 않습니다. 번역되지 않은 페이지로 이동하는 언어 버튼도 표시하지 않습니다. [다국어 작업 안내](docs/I18N.md)에 경로, 번역 키, 공개 절차를 정리했습니다.
...
브라우저 테스트는 임시 포트에서 로컬 서버를 시작하고 종료합니다. 여섯 페이지의 헤더와 320/360/390/430/768/1024/1280/1440px 레이아웃, 상품별 사이즈·주문 실패 재시도·이전 장바구니 이관, 서울 자정 경계·열어 둔 창의 마감 처리, 번역 사전과 경로를 확인합니다. 서버 테스트는 유효하지 않은 공연·사이즈·수량을 거절하고 실제 메일 본문에 사이즈가 들어가는지 검사합니다. 신청 API는 모의 응답을 사용하며 실제 이메일은 보내지 않습니다. 본문 레이아웃 확인을 위해 Tailwind CDN에 접근할 수 있어야 합니다.
