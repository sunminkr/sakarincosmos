...
홈 첫 화면은 `assets/images/band/sakarin-cosmos-live.jpeg`의 밴드 사진을 사용합니다. 세 언어의 홈에서 같은 파일을 공유하며, 모바일에서도 세 멤버가 잘리지 않도록 사진 전체를 표시합니다. 하단 소개 영역의 배경에는 기존 디자인용 예시가 남아 있습니다. 공연·상품 목록은 전달받은 실제 정보로 교체했습니다. Objects의 로고 티셔츠 사진은 `assets/images/products/sakarin-cosmos-logo-t-shirt/`에서 관리합니다. `front.png`는 대표 사진, `logo-detail.png`와 `worn.png`는 펼쳐 보는 상세 사진이며, 세 언어 모두 같은 원본을 공유합니다. 사진의 비율을 유지하고 원본 링크를 제공하며 JavaScript 없이도 상세 사진을 열 수 있습니다. PRESS KIT와 SMTP 운영 설정은 후속 작업입니다.
