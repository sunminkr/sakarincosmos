...
2026-09-21에 확인한 공개 목록을 등록했습니다. 번역은 보류하며, 공식 계정의 원문과 임베드를 사용합니다.

| 계정 | 등록한 콘텐츠 |
| --- | --- |
| [Instagram @sakarincosmos](https://www.instagram.com/sakarincosmos/) | 프로필에 표시된 사진·릴스 6개. 협업 계정 게시물 2개 포함 |
...
`data/media.json`의 `entries`에서 해당 항목 전체를 삭제하거나, `"enabled": true`를 `"enabled": false`로 바꾼 뒤 새로고침합니다. 사이트에서만 제외되며 원본 SNS 게시물은 변경하지 않습니다.

홈·음악 페이지·아카이브가 같은 목록을 사용하므로 한 번만 수정하면 됩니다. 홈 최신 음악·영상, 최근 기록과 필터 개수도 자동으로 갱신됩니다. 마지막 쉼표 등 JSON 문법에 주의하세요.

| 항목 ID | 콘텐츠 |
| --- | --- |
| `youtube-i7-C-GW8MZ8` | 그럼에도 계속되는 것 · 클럽 빵 전체 공연 영상 |
| `instagram-DdiB9ELGaCP` | 9월 20일 사진, 프로필 첫 번째 |
| `instagram-DdiBj_lzZsZ` | 9월 20일 사진, 프로필 두 번째 |
| `instagram-DdiBgBfvueh` | 9월 20일 사진, 프로필 세 번째 |
...
- 상세 팝업을 열 필요 없이 카드 안에서 바로 임베드를 볼 수 있습니다. 필터 변경으로 빠지는 iframe은 제거해 재생을 중지합니다.
...
| `description` | 설명. Instagram 본문은 공식 임베드 안에 표시되므로 별도 복사하지 않음 |
...
빌드는 필요 없습니다. URL 대신 전체 iframe 코드를 붙이지 않습니다. SoundCloud의 공유 → Embed 코드에 들어 있는 `src` 주소는 지원하며 자동 재생은 끕니다. 직접 음원 파일과 API 키는 사용하지 않습니다.

Instagram은 공식 `embed.js`로 표시하고, 서비스가 본문과 이미지에 맞춰 높이를 계산합니다. 모바일에서 기본 최소 너비 326px이 화면을 넘지 않도록 조정했습니다. SoundCloud 트랙은 높이 166px, 플레이리스트는 450px입니다. YouTube는 16:9를 기본으로 하되 높이 200px 이상을 확보합니다.
...
`npm test`는 실제 목록의 정렬·서비스별 필터·개수, 삭제/숨김의 전체 페이지 반영, 오류와 빈 목록, 320/390/768/1440px 임베드 크기, 공식 SNS 링크 및 기존 픽업 기능을 검사합니다. 자동 회귀 검사에서는 외부 플레이어를 모의 응답으로 대체하며, 별도로 실제 8개 임베드 표시, Instagram의 320px 내부 레이아웃, SoundCloud·YouTube 재생 시작과 재생 시간 진행도 확인했습니다.

홈 배경·상품 이미지와 공연·상품 목록에는 기존 디자인용 예시가 남아 있습니다. 실제 공연 및 물품 자료를 받으면 별도로 교체해야 합니다. 소개·PRESS KIT와 SMTP 운영 설정도 후속 작업입니다.
